...
final class Obfuscation {
    ...
    static byte[] remap(String internalName, byte[] classfile) {
        try {
            ...
            reader.accept(new RemappingClassAdapter(writer, active),
    ...
    private static void buildRemapper() {
        String table = naming == Naming.NOTCH ? "/resolutions/obfuscation.tsv" : "/resolutions/mcp.tsv";
        if (!load(table)) {
            return;
        }
    ...
    private static boolean load(String resource) {
        ...
        if (in == null) {
            Log.line(resource + " is missing from the jar.");
            return false;
        }
