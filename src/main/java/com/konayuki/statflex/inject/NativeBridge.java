...
public final class NativeBridge {
    ...
    private static void checkCallbacks() {
        int checked = 0;
        int wrong = 0;
        List<HookRegistry.Hook> dropped = new ArrayList<HookRegistry.Hook>();
        for (String owner : HookRegistry.owners()) {
            for (HookRegistry.Hook hook : HookRegistry.forOwner(owner)) {
                ...
                if (problem != null) {
                    wrong++;
                    log("Bad callback " + hook.owner + "." + hook.method + " @" + hook.position + " -> " + problem + " (hook disabled)");
                    dropped.add(hook);
                }
            }
        }
        for (HookRegistry.Hook hook : dropped) {
            HookRegistry.remove(hook);
        }
        log("Callbacks checked: " + checked + (wrong == 0 ? ", all present" : ", " + wrong + " DISABLED"));
    }
    ...
    private static void warmUp() {
        try {
            ...
            Obfuscation.prepare();
            byte[] sample = readOwn("com/konayuki/statflex/inject/Log.class");
            if (sample != null) {
                transform("com/konayuki/statflex/inject/Log", sample);
                transformer.warmUp(sample);
            }
            log("Warmup complete: " + (ourClasses == null ? 0 : ourClasses.size())
                    + " own classes known");
        } catch (Throwable t) {
            Log.throwable("Warmup failed", t);
        }
    }
    ...
    private static Set<String> readOwnJar() {
        ...
        try {
            ...
            for (Enumeration<JarEntry> entries = jar.entries(); entries.hasMoreElements(); ) {
                ...
                if (name.startsWith("com/konayuki/statflex/shadow/")
    ...
    private static void dump(String name, byte[] bytes) {
        try {
            File dir = new File(System.getProperty("java.io.tmpdir"), "statflex-dump");
