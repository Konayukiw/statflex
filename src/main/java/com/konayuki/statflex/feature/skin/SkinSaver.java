package com.konayuki.statflex.feature.skin;
...
public class SkinSaver {
    ...
    private static void savePlayerSkin(String playerName, boolean useNpcSkin) {
        try {
            ...
            if (info == null) {
                ChatManager.send("§8[§cS§8]§7 Player not found: " + playerName);
                return;
            }
            ...
            if (textureJson == null) {
                ChatManager.send("§8[§cS§8]§7 Failed to fetch skin data");
                return;
            }
            ...
            if (textures == null || !textures.has("SKIN")) {
                ChatManager.send("§8[§cS§8]§7 No skin found.");
                return;
            }
            ...
            e.printStackTrace();
            ChatManager.send(Messages.UNEXPECTED_ERROR);
        }
    }
    ...
    private static void downloadAndSaveSkin(String urlStr, String name, UUID uuid) {
        try {
            ...
            e.printStackTrace();
            ChatManager.send("§8[§cS§8]§7 Failed to download skin");
        }
    }
