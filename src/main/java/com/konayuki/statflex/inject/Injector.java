...
public final class Injector {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            Debug.log("Cannot work out where this jar is: " + t);
            return;
        }
        if (!self.getName().endsWith(".jar")) {
            Debug.log("Run this from the built jar, not from loose classes: " + self);
            return;
        }
        Class<?> vmClass = attachClass();
        if (vmClass == null) {
            Debug.log("No attach API available.");
            Debug.log("Run this with a JDK, not a JRE.");
            return;
        }
        try {
            ...
            try {
                ...
                Debug.log("Agent loaded. Everything else appears in the game's own log");
            } finally {
                vmClass.getMethod("detach").invoke(vm);
            }
        } catch (Throwable t) {
            Throwable cause = t.getCause() == null ? t : t.getCause();
            Debug.log("Attach failed: " + cause);
            Debug.log("Either the target runs with -XX:+DisableAttachMechanism, or it");
            Debug.log("Belongs to another user, or its Java version is too far from this");
            Debug.log("one: attaching from a modern JDK to a Java 8 game fails with");
            Debug.log("\"Failed to load agent library\". Run this on the same Java the");
            Debug.log("game uses.");
        }
    }
    ...
    private static String choose(Class<?> vmClass) throws Exception {
        ...
        if (likely.size() == 1) {
            ...
            Debug.log("One candidate, using it: " + id);
            return id;
        }
        Debug.log("Pass a pid. Java processes the attach API can see:");
        if (all.isEmpty()) {
            Debug.log("  (none)");
        }
        for (Object vm : all) {
            String name = displayName(vm);
            Debug.log("  " + id(vm) + "  " + shorten(name)
    ...
    private static void listProcessesTheAttachApiMissed(List<Object> listed) {
        ...
        System.out.println();
        Debug.log("Java processes the OS sees but the attach API does not:");
        for (String[] row : found) {
            Debug.log("  " + row[0] + "  " + row[1]);
        }
        System.out.println();
        Debug.log("A JVM started with -XX:-UsePerfData is missing from the list above");
        Debug.log("but can usually still be attached to. Try the pid directly:");
        Debug.log("  java -jar <this jar> <pid>");
    }
    ...
    private static boolean looksLikeMinecraft(String displayName) {
        ...
        if (lower.contains("gradle") || lower.contains("statflex") || lower.contains("injector")) {
            return false;
        }
