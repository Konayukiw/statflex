package com.konayuki.statflex.stats.skywars;
        ...
        new Thread(() -> {
            try {
                ...
                if (apiKey.equals("N/A")) {
                    ChatManager.send(Messages.INVALID_API);
                    return;
                }

                ProfileHandler.PlayerInfo info = ProfileHandler.getPlayerInfo(playerName);
                if (info == null) {
                    ChatManager.send(Messages.PLAYER_NOT_FOUND);
                    return;
                }
                ...
                if (!response.has("success") || !response.get("success").getAsBoolean()) {
                    String cause = response.has("cause") && !response.get("cause").isJsonNull()
                            ...
                            : "Unknown error";
                    ChatManager.send(Messages.FETCH_ERROR + cause);
                    return;
                }
                ...
                if (statsRoot == null || !statsRoot.has("SkyWars") || !statsRoot.get("SkyWars").isJsonObject()) {
                    ChatManager.send(Messages.FETCH_ERROR + " No stats found for name" + playerName);
                    return;
                }
                ...
                String coloredPlayerName = Formatter.getColoredPlayerName(player, properName);
                ...
                if (mode != null) {
                    ...
                    ChatManager.send(String.format(Messages.SKYWARS_STATS + "§7[§e%s§7]", displayMode));
                    ChatManager.send(String.format("§c || %s %s §7| Wins: %s §7| KDR: %s",
                            levelFormatted, coloredPlayerName, formattedWins, formattedKDR));
                } else {
                    ChatManager.send(String.format(Messages.SKYWARS_STATS));
                    ChatManager.send(String.format("§c || %s %s §7| Wins: %s §7| KDR: %s",
                            levelFormatted, coloredPlayerName, formattedWins, formattedKDR));
                }
