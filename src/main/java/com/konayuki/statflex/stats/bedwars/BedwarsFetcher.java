    ...
    public static void fetchStats(String inputName, String mode) {
        new Thread(() -> {
            try {
                ...
                ProfileHandler.PlayerInfo info = ProfileHandler.getPlayerInfo(inputName.toLowerCase());
                if (info == null) {
                ...
                if (!response.has("success") || !response.get("success").getAsBoolean()) {
                    ...
                    if (lower.contains("invalid") || lower.contains("api key")) {
                        ...
                        ChatManager.send(Messages.FETCH_ERROR + properName + "§7| " + cause);
                    }
                    return;
                }
                ...
                if (mode != null && !mode.isEmpty()) {
                    ...
                    if (key == null) {
                        ChatManager.send(Messages.INVALID_MODE + mode);
                        return;
                    }
                ...
                if (mode != null && !mode.isEmpty()) {
                    ...
                    ChatManager.send(String.format(Messages.BEDWARS_STATS + "§7[§e%s§7]", displayMode));
                    ChatManager.send(String.format("§c || %s %s §7| Finals: %s §7| FKDR: %s",
