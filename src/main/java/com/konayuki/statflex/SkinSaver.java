...
public class SkinSaver {
    ...
    private static void savePlayerSkin(String playerName, boolean useNpcSkin) {
        try {
            ...
            if (textureJson == null) {
                sendChat("§8[§cS§8]§7 Failed to fetch skin data");
                return;
            }
    ...
    private static String fetchTextureJson(UUID uuid) {
        try {
            ...
            if (Fetcher.ignoreCertificates && conn instanceof HttpsURLConnection) {
                GetUUID.trustAllCertificates((HttpsURLConnection) conn);
            }
    ...
    private static void downloadAndSaveSkin(String urlStr, String name, UUID uuid) {
        try {
            ...
            File downloadDir = SettingsManager.getInstance().getSkinSaveDir();
