...
public class DuelsFetcherUpdated {
    ...
    public static void fetchStats(String inputName, String mode, boolean auto) {
        new Thread(() -> {
            try {
                ...
                if (auto) {
                    ChatManager.send(String.format("§8[§cS§8]§6 %s %s §7| Wins: %s §7| WLR: %s",
                            modeAndTitle, coloredPlayerName, formattedWins, coloredWLR));
                } else {
                    if (mode == null) {
                        ChatManager.send(Messages.DUELS_STATS);
                    } else {
                        ChatManager.send(String.format(Messages.DUELS_STATS + "§7[§e%s§7]", modeDisplay));
                    }
                    ChatManager.send(String.format("§c ||§6 %s %s §7| Wins: %s §7| WLR: %s",
                            modeAndTitle, coloredPlayerName, formattedWins, coloredWLR));
                }
